...
%(indent)s</%(qName)s>%(tailText)s%(lf)s'''
        ...
        if subTreeStringList == [] and elementValueString == "":
            ...
            if subTreeStringList != []:
                subTreeString = "".join (subTreeStringList)
            else:
                subTreeString = ""
...
#++++++++++++ methods concerning the children of the current node ++++++++++++++++++++++++
    ...
    def getChildren (self, tagFilter=None):
        ...
        if tagFilter in (None, '*', (None, '*')):
            ...
            # handle (namespace, '*')
            children = list(filter(lambda child:child.xmlIfExtElementWrapper.getNamespaceURI() == tagFilter[0], 
        ...
        return list(map(lambda child: child.xmlIfExtElementWrapper, children))
    ...
    def getFirstChildWithKey (self, tagFilter=None, keyAttr=None, keyValue=None):
        ...
        """
        children = self.getChildren(tagFilter)
        childrenWithKey = list(filter(lambda child:child[keyAttr]==keyValue, children))
        if childrenWithKey != []:
    ...
    def getElementsByTagName (self, tagFilter=None):
        ...
        if tagFilter in (None, '*', (None, '*'), (None, None)):
            ...
            # handle (namespace, '*')
            descendants = list(filter(lambda desc:desc.xmlIfExtElementWrapper.getNamespaceURI() == tagFilter[0], 
...
#++++++++++++ limited XPath support ++++++++++++++++++++
        ...
        for xRelPath in xPathList:
            ...
            for child in childList:
                try:
                    ...
                    childKey = child
                if not (childKey in completeChildDict):
    ...
    def __updateAttrMaxLengthDict (self, attrMaxLengthDict):
        ...
        for attrName, attrValue in self.getAttributeDict().items():
            ...
            if not (attrName in attrMaxLengthDict):
    ...
    def __useCaching(self):
        # **** ESS LLC EDITS ****
        # ** There appears to be a bug in treeWrapper.useCaching or in this class wrapper implementation.
        # ** Not clear the cause, so just turn it off caching, and magically things work as they should.
        # ** This means a performance hit, but better than the class not working properly. 
        #return self.treeWrapper.useCaching() # returns 1 if caching is on
        # **** ESS LLC EDITS ****
        return 0
