        ...
        if (inputRootNsName in self.xsdElementDict):
            ...
            if not self.errorHandler.hasErrors():
                ...
                for idref in self.xsdIdRefDict.keys():
                    if not (idref in self.xsdIdDict):
        ...
        for redefineNode in rootNode.getChildrenNS(namespaceURI, "redefine"):
            ...
            for childNode in redefineNode.getChildren():
                ...
                if (redefType in xsdDict):
                    ...
                    if not (orgRedefType in xsdDict):
    ...
    def _updateLookupTables (self, rootNode, lookupDict):
        ...
        for localName, lookupDictName in schemaTagDict.items():
            for node in rootNode.getChildrenNS(XSD_NAMESPACE, localName):
                ...
                if not ((targetNamespace, node.getAttribute("name")) in lookupDict[lookupDictName]):
        ...
        for identConstrTagName in ("unique", "key", "keyref"):
            ...
            for identConstrNode in identConstrNodeList:
                ...
                if not (identConstrNsLocalName in lookupDict["IdentityConstrDict"]):
...
#            self._checkMixed (xsdNode, inputNode)
    ...
    def _minmaxOccurs(self,xsdNode):
    ...
    def _checkList (self, elementMethod, xsdNode, inputParentNode, inputNodeList, currIndex):
        minOccurs,maxOccurs = self._minmaxOccurs(xsdNode)
        ...
        if occurs == 0 and minOccurs > 0 and lastError is not None:
            raise lastError
            '''
            expInputTagName = xsdNode.getAttribute("name")
            if expInputTagName == None:
                expInputTagName = xsdNode.getQNameAttribute("ref")
            raise TagException('The node {0} has no data. There is a required minimum of {1}.'.format(expInputTagName,minOccurs))
            '''
    ...
    # inputNodeList is actually an iterable map object, not a list
    #
    def _checkElementTag (self, xsdNode, inputParentNode, inputNodeList, currIndex):
        if xsdNode.hasAttribute("ref"):
            ...
            if minOccurs==0:
                # **** ESS EDITS ****
                # a reference element to another, possible complex, type
                # may have no occurrances (i.e. minOccurs = 0), so check that
                children=xsdNode.getChildren()
                # xsdNode.getChildren() returns a map object in Python 3, so convert to a list so we can get length
                # the below method is slightly faster than casting using list()
                nodeList=[x for x in children]
                if len(nodeList)==0:
                    # no elements of this type present in the XML file, which is allowed, so return currIndex unchanged
                    return currIndex
                # **** ESS LLC EDITS ****
            refNsName = xsdNode.getQNameAttribute("ref")
            #currIndex = self._checkParticle (self.xsdElementDict[refNsName], inputParentNode, inputNodeList, currIndex)
            ...
            # **** ESS EDITS ****
            # inputNodeList is actually a map object in Python 3
            # convert map to a list so we can get length
            # the below method is slightly faster than casting using list()
            nodeList=[x for x in inputNodeList]
            if currIndex >= len(nodeList):
                raise TagException ("Missing child tag %s in tag %s!" %(repr(nameAttr), repr(inputParentNode.getTagName())), inputParentNode, 1)
            # **** ESS EDITS ****
            inputNode = nodeList[currIndex]
            ...
            if (xsdNode.getFirstChild() == None and 
                ...
                for inputChild in inputNode.getChildren():
                    try:
                        if (inputChild in self.xsdElementDict.getNsName()):
            ...
            if not inputNode.hasAttribute((XSI_NAMESPACE, "type")):
                ...
                if not (typeNsName in self.xsdTypeDict):
    ...
    def _checkAttributeTags (self, parentNode, xsdNode, inputNode, validAttrDict):
        ...
        # If the attribute in the XML file is found in the XSD file, and all was either correct or errors were recorded there.
        # And if inputAttrDict was cleared in _checkAttributeTag() and the rest of the code sould not run
        # thus letting the program continue processing without recording any errors below.
        for inputAttribute in inputAttrDict.keys():
            ...
            elif parentNode.getAttributeOrDefault (inputAttribute, False):
                pass # for each attribute in XML input, confirms that the XSD schema also has it
            else:
                self._addError ("Unexpected attribute %s in Tag %s!" %(repr(inputAttribute), repr(inputNode.getTagName())), inputNode)
    ...
    def _checkAttributeTag (self, qAttrName, xsdAttrNode, xsdAttrRefNode, inputNode, inputAttrDict):
        ...
        elif ((None, qAttrName[1]), inputAttrDict) and qAttrName[0] == targetNamespace:
    ...
    def _checkKeyRefConstraint (self, keyrefNode, inputNode):
        ...
        for selectorNode in selectorNodeList:
            ...
            keyValue = tuple(keyValue)
            if keyValue != ():
                if not (keyValue in self.xsdIdentityConstrDict[keyReference]["ValueDict"]):
    ...
    def _setBaseTypes (self, xsdNode):
        if xsdNode.getAttribute("BaseTypes") != None:
            ...
            baseTypeList = list(map (lambda basetype: NsNameTupleFactory(basetype), baseTypes))
            if baseTypeList != []:
                return baseTypeList
            else:
                return None
